...
def _pin_to_resources(rank_queue: Queue, total_ranks: int):
    ...
    #_ppex: ProcessPoolExecutor | None = None
    #
    #ngpus: int = 12#get_num_intel_gpu_max_tiles()
    #
    ...
    if "ZE_AFFINITY_MASK" not in os.environ:
        os.environ["ZE_AFFINITY_MASK"] = f'{gpu}'#str(rank)
        os.environ["ZEX_NUMBER_OF_CCS"]="0:2,1:2,2:2,3:2,4:2,5:2"
        print(os.environ["ZE_AFFINITY_MASK"])
        logger.info(f"Pinned to GPU: {rank}")
    else:
        logger.info("ZE_AFFINITY_MASK is already set")
        return  # Don't bother with the CPU affinity either
    #os.environ['ZE_AFFINITY_MASK'] = f'{gpu}.{ins}'
    #env = os.environ.copy()
    #env['ZE_AFFINITY_MASK'] = f'{gpu}.{ins}'
    #print(env['ZE_AFFINITY_MASK'])
    print(f"Pinned to GPU {rank}")
...
def __pin_to_resources(rank_queue: Queue, total_ranks: int):
    """Pop the rank off the list queue to get your rank, then assign to associated resources """
    # Get my rank
    rank = rank_queue.get()
...
def make_pinned_process_pool(n_gpus: int = 12, tasks_per_gpu: int = 2) -> ProcessPoolExecutor:
...
def get_persistent_process_pool(n_gpus: int = 12, tasks_per_gpu: int = 2) -> ProcessPoolExecutor:
